...
const PREVIEW_MASK = "linear-gradient(to bottom, #000 45%, transparent)";
  ...
  return (
    <SpotlightCard className="h-full">
      <div className="relative flex h-full flex-col">
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none relative h-32 overflow-hidden border-b border-ink-900 bg-ink-900/40 px-4 py-3 font-[family-name:var(--font-mono)] text-[11px] leading-5"
        ...
        >
          {previewLines(room).map((line, i) => (
            <div key={i} className="flex gap-3 whitespace-pre">
              <span className="w-3 shrink-0 text-right text-ink-700">{i + 1}</span>
              <span className="truncate text-ink-400">{line || " "}</span>
            </div>
          ))}
        </div>
        ...
        <div className="pointer-events-none relative flex flex-1 flex-col p-4">
          <div className="flex items-start justify-between gap-3">
            <h3 className="truncate text-sm font-medium text-ink-100">{room.name}</h3>
          ...
          <div className="mt-auto flex items-center justify-between gap-2 pt-4">
            <div className="flex min-w-0 items-center gap-2 text-xs text-ink-500">
              <span className="shrink-0 rounded-md border border-ink-800 bg-ink-900 px-1.5 py-0.5 text-[10px] text-ink-400">
              ...
              <button
                ...
                className={`pointer-events-auto relative z-10 flex h-7 shrink-0 items-center gap-1 rounded-lg px-2 text-xs transition-colors ${
                  confirmingDelete
                    ...
                    : "text-ink-600 hover:bg-ink-900 hover:text-red-400"
                }`}
              >
